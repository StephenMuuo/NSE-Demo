...
with tab2:
    ...
    for stock in df['Name'].unique():
        ...
        fig.add_trace(go.Scatter(x=stock_df['Date'], y=stock_df['SMA_10'], 
                             name=f'{stock} - SMA 10', line=dict(dash='dot')))
        fig.add_trace(go.Scatter(x=stock_df['Date'], y=stock_df['SMA_50'], 
                             name=f'{stock} - SMA 50', line=dict(dash='dash'))))
        fig.add_trace(go.Scatter(x=stock_df['Date'], y=stock_df['Day Price'], 
                             name=f'{stock} - Price', line=dict(width=2)))
